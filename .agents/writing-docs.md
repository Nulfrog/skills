...
Only skills that exist upstream have an `aihero.dev` page. **A skill that exists only in this fork is linked at its `SKILL.md` on GitHub**, at `https://github.com/Nulfrog/skills/blob/main/skills/<bucket>/<name>/SKILL.md`, because `https://aihero.dev/skills-<name>` would 404 for it. That applies wherever the fork-only skill is named, including from the pages of upstream skills.
