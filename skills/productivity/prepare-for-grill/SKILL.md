...
- One decision tree, not several tangled together.
- Small enough to reach shared understanding before the **smart zone** runs out (~120k tokens; see `/ask-matt`).
- Several independent decision trees should become separate grill prompts. A huge, foggy effort whose route is not yet clear belongs in `/wayfinder`; an already-settled design whose implementation spans sessions belongs in `/to-spec` → `/to-tickets`.
...
<sharpened, unambiguous, right-sized prompt; one decision tree>
```
