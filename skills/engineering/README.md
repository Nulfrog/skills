...
- **[setup-nulfrog-skills](./setup-nulfrog-skills/SKILL.md)** — Apply Nulfrog's local setup conventions on top of `setup-matt-pocock-skills`: AGENTS.md as canonical, concise-communication wiring, and a `spec` provenance label. Run once, after the base setup.
...
- **[implement](./implement/SKILL.md)** — Build a spec or ticket with `/tdd`, then close out with `/code-review` before committing.
...
- **[code-review](./code-review/SKILL.md)** — Two-axis review of a diff: repository standards and fidelity to the originating spec, run as parallel sub-agents.
- **[resolving-merge-conflicts](./resolving-merge-conflicts/SKILL.md)** — Resolve an in-progress merge or rebase from primary-source intent, then run the repository's checks.
