...
2. **Ensure `.gitattributes`**: if there's no `.gitattributes` at the repo root,
   create one containing `* text=auto eol=lf` and stage it with this commit. This
   normalises line endings and prevents CRLF/LF churn across platforms (Windows
   especially). If one already exists, leave it as-is.
3. **Stage deliberately** — don't blindly `git add -A`. Review untracked files;
...
4. **Commit** (see OS-safe commit below).
5. **Pull** (`git pull --rebase`): run immediately after commit to integrate
...
6. **Verify**: `git status` after pull; report tree state + commits ahead/behind
   remote.
7. **Suggest `.gitignore` update**: if step 3 found files that obviously
   shouldn't be tracked, list them and offer to add the matching `.gitignore`
   entries. Only edit `.gitignore` if the user agrees.
8. **Ask to push**: end your reply by asking whether the user wants to push.
