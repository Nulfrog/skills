...
description: Configure this repo for the engineering skills — set up its issue tracker, triage label vocabulary, domain doc layout, and a concise communication style (an `## Agent skills` block in `AGENTS.md` imported by `CLAUDE.md` via `@AGENTS.md`, a `.cursor/rules/concise-communication.mdc` rule, and a `.claude/settings.json` UserPromptSubmit hook). Run once before first use of the other engineering skills.
