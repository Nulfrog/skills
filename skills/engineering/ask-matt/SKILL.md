...
1. **`/grill-with-docs`** — sharpen the idea by interview. Start here when you **have a codebase**: it's stateful, retaining what it learns in `CONTEXT.md` and ADRs. (No codebase? Use `/grill-me` — see Standalone.) Shaky idea — ambiguous, oversized, or leaning on external links/docs? **`/prepare-for-grill`** first for feedback (see Standalone).
